...
const explain_notice = new Dialog(document.getElementById('explain_notice'));
...
const location_pin_html = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-map-pin-icon lucide-map-pin"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>`
...
function share_location(){
    ...
    const current_copy_url = `https://mofom.net/cacs/weather?latitude=${current_city.coordinate[1]}&longitude=${current_city.coordinate[0]}`;
...
async function load_display_content(content) {
    ...
    const time_err_placeholder = '0000-00-00T00:00:00'
    ...
    const city_info = {
        ...
        sunlight: {
            transit_time: new DateNoTimeZone(content.properties.astronomicalData.transit ?? time_err_placeholder),

            normal_sunrise: new DateNoTimeZone(content.properties.astronomicalData.sunrise ?? time_err_placeholder),
            normal_sunset: new DateNoTimeZone(content.properties.astronomicalData.sunset ?? time_err_placeholder),

            civil_twilight_begin: new DateNoTimeZone(content.properties.astronomicalData.civilTwilightBegin ?? time_err_placeholder),
            civil_twilight_end: new DateNoTimeZone(content.properties.astronomicalData.civilTwilightEnd ?? time_err_placeholder),

            nautical_twilight_begin: new DateNoTimeZone(content.properties.astronomicalData.nauticalTwilightBegin ?? time_err_placeholder),
            nautical_twilight_end: new DateNoTimeZone(content.properties.astronomicalData.nauticalTwilightEnd ?? time_err_placeholder),

            astronomical_twilight_begin: new DateNoTimeZone(content.properties.astronomicalData.astronomicalTwilightBegin ?? time_err_placeholder),
            astronomical_twilight_end: new DateNoTimeZone(content.properties.astronomicalData.astronomicalTwilightEnd ?? time_err_placeholder),
        },
    ...
    put_content_to_page(display_cityname, `${content.location_pin ? location_pin_html:""}${city_info.name}, ${city_info.state}`, true)
...
async function init() {
    ...
    try{
        ...
        display_error_dl.open(`Failed to get your location!<a href="./searchcity/index.html"><button class="delete_saved_city_btn">Search For City</button></a>`, true);
    }
